...
const storage = multer.diskStorage({ // J'enregistre dans le disk
    destination: (req, file, callback) => { // Je défini la destination des images qui seront stocker dans le fichier 'images'
        callback(null, 'images')
    },
    filename: (req, file, callback) => { // Je défini le nom du fichier image
        const name = file.originalname.split(' ').join('_') /* Je crée un tableau autour des espaces à l'aide de la méthode 'split'
                                                            Puis je remplace les espaces par des underscored */
